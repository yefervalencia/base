...
class BaseHandlerTest {
    ...
    void handleSuccessShouldReturn200WithBody() {
        WebTestClient client = WebTestClient.bindToRouterFunction(
                route(GET("/success"), request -> handler.handleSuccess("ok"))
        ).build();
    ...
    void handleCreatedShouldReturn201WithBody() {
        WebTestClient client = WebTestClient.bindToRouterFunction(
                route(GET("/created"), request -> handler.handleCreated("created"))
        ).build();
    ...
    void handleNoContentShouldReturn204() {
        WebTestClient client = WebTestClient.bindToRouterFunction(
                route(GET("/nocontent"), request -> handler.handleNoContent())
        ).build();
    ...
    void handleBadRequestShouldReturn400WithErrorBody() {
        WebTestClient client = WebTestClient.bindToRouterFunction(
                route(GET("/bad"), request -> handler.handleBadRequest("invalid"))
        ).build();
